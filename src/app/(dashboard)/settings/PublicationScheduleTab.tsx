...
/**
 * Reusable weekly day × time grid + max-per-day + timezone editor for any
 * PublicationSchedule-shaped block (used by both the social and video sections).
 * `idPrefix` keeps the per-cell aria-labels unique across grids on the page.
 */
function ScheduleGrid({
  ...
  onChange: (next: PublicationSchedule) => void;
  idPrefix: string;
}) {
  const columns = columnsFor(schedule);

  function toggleSlot(day: WeekdayKey, time: string) {
    const slots = schedule.slots.map((s) => ({ day: s.day, times: [...s.times] }));
    ...
    onChange({ ...schedule, slots: cleaned });
  }

  return (
    <div className={schedule.enabled ? "" : "opacity-50 pointer-events-none"}>
      ...
      <div className="overflow-x-auto">
        <table className="text-sm border-separate border-spacing-1">
          ...
          <tbody>
            {DAYS.map((d) => {
              const active = timesOf(schedule, d.key);
              return (
                <tr key={d.key}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-5">
        <div>
          <label className="block text-sm text-gray-400 mb-1">
            Max par jour : <span className="text-gray-200 font-medium">{schedule.max_per_day}</span>
          </label>
          <input
            type="range"
            min={1}
            ...
            onChange={(e) => onChange({ ...schedule, max_per_day: Number(e.target.value) })}
        ...
        <div>
          ...
          >
            {(TIMEZONES.includes(schedule.timezone) ? TIMEZONES : [schedule.timezone, ...TIMEZONES]).map((tz) => (
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (d.success) {
        ...
        setVideo(d.data.video_schedule);
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/settings/schedule", {
        ...
        body: JSON.stringify({ publication_schedule: pub, blog_schedule: blog, video_schedule: video }),
      });
      const d = await res.json();
      if (d.success) {
        setPub(d.data.publication_schedule);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Toggle on={pub.enabled} onClick={() => setPub({ ...pub, enabled: !pub.enabled })} />
        </div>
        <ScheduleGrid schedule={pub} onChange={setPub} idPrefix="social" />
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-white font-semibold">Vidéos</h3>
            <p className="text-xs text-gray-500 mt-0.5">Reels — horaire indépendant des posts sociaux et du blog.</p>
          </div>
