...
function formToOpts(f: GenForm): Record<string, unknown> {
  ...
  switch (f.template) {
    case "SHOWCASE":
      // SHOWCASE renders one SKU's multi-angle series — use the first entered SKU.
...
export default function SlideshowSettingsTab() {
  ...
  // Video dry-run manifest (parsed) + whether a manifest preview has succeeded
  // for the CURRENT params (gates the Generate button).
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (d.success) {
        ...
        if (d.data.slideshow_settings?.platform) {
          setForm((prev) => ({ ...prev, platform: d.data.slideshow_settings.platform, ratio: d.data.slideshow_settings.default_ratio ?? prev.ratio }));
        }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-5">
        ...
        <p className="text-xs text-gray-500 mb-4">
          ...
          le MP4 et mets-le en file de publication.
        </p>
