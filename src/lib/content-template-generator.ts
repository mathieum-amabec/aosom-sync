/**
 * Content-template draft generation, callable IN-PROCESS.
 *
 * This logic used to live only inside the `POST /api/social/content/generate`
 * route handler, and `/api/cron/content` reached it by `fetch()`ing its own
 * origin. That self-call never arrived: Vercel Cron invokes the function on the
 * *deployment* URL, which SSO Deployment Protection guards, so the platform
 * answered 401 at the edge before the route's own CRON_SECRET check ran. Every
 * run from 2026-07-29 to 2026-08-26 failed that way — 13 runs, 13 errors, zero
 * drafts — while the generator itself was fine the whole time (a direct call to
 * the production alias with the same secret generates a draft on the first try).
 *
 * `lib/blog-generator.ts` already learned this lesson; `/api/cron/blog` was moved
 * in-process for exactly this reason and left a note saying so. The content cron
 * never got the same treatment. It has it now: the cron calls
 * `generateContentTemplateDraft()` directly and no longer crosses the network to
 * reach code in its own bundle.
 *
 * The HTTP route stays the operator-facing entry point and is a thin wrapper:
 * auth, rate limit, validate, call this, map errors to status codes.
 */

import {
  getContentTemplateBySlug,
  createFacebookDraft,
  getAnyProductSku,
  selectCompatibleHooks,
  getRecentlyUsedHookIds,
  recordHookUsage,
} from "@/lib/database";
...
/**
 * A generation failure with the HTTP status the route should answer.
 *
 * Carrying the status on the error is what lets the route stay a thin wrapper
 * while still returning 404 for an unknown slug, 422 for an inactive template,
 * 503 for an exhausted hook pool and 502 for an empty completion — instead of
 * flattening every cause into a 500. The cron ignores `status` and reports
 * `message`, which is what lands in `cron_runs.detail`.
 */
...
- N'écris PAS le mot "Accroche:" ni aucune étiquette devant l'accroche — écris directement la phrase.
...
- Do NOT write "Hook:" or any label in front of the hook — write the sentence itself.
 ...
 * ("Here's a Facebook post for your product:"), a platform-label title
 * ("Post Facebook 🌿"), an `Accroche:` label in front of the hook, a markdown
 * "---" rule, or wrap lines in **bold**. Relying on the prompt alone is not
 * enough — without this strip a single disobedient generation gets persisted
 * verbatim into the draft and then published. Exported for unit testing.
 */
export function stripScaffold(text: string): string {
  ...
  //    then a leading `Accroche:` / `Hook:` label (keeping the hook itself).
...
/**
 * Generate one content_template draft in one language and persist it.
 *
 * Throws `ContentGenerationError` with the status the HTTP route should answer;
 * any other throw is an unexpected fault the caller turns into a 500.
 */
export async function generateContentTemplateDraft(
  templateSlug: string,
  language: ContentLanguage,
): Promise<GeneratedContentDraft> {
  const template = await getContentTemplateBySlug(templateSlug);
  if (!template) {
    throw new ContentGenerationError(`Template '${templateSlug}' not found`, 404);
  }
  if (!template.active) {
    throw new ContentGenerationError(`Template '${templateSlug}' is not active`, 422);
  }
  ...
  if (template.mode !== "generative_seeded") {
    ...
    if (hookCandidates.length === 0) {
      throw new ContentGenerationError(
        `No ${hookLang} hooks available in pool — cannot generate hook_seeded post`,
        503,
      );
    }
  ...
  if (!postText) {
    throw new ContentGenerationError("Claude returned an empty response", 502);
  }

  // facebook_drafts.sku has FK on products(sku) — content template drafts use any real SKU
  const sku = await getAnyProductSku();
  if (!sku) {
    throw new ContentGenerationError("No products in catalog — cannot create draft", 503);
  }
  ...
  try {
    ...
    console.warn(`[content-gen] Unsplash fetch failed (non-fatal):`, imgErr instanceof Error ? imgErr.message : imgErr);
  }
  ...
  // must not turn a successful creation into a throw (which would make the cron
  // retry and risk a duplicate draft).
  if (hookChosen) {
    try {
      await recordHookUsage(hookChosen.id, draftId);
    } catch (recordErr) {
      console.error(`[content-gen] recordHookUsage failed for hook ${hookChosen.id} / draft ${draftId}:`, recordErr);
    }
  }
