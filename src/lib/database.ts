...
async function _initSchemaImpl(): Promise<void> {
  ...
  const schemaStatements = [
    ...
    `CREATE TABLE IF NOT EXISTS csv_cache_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    `CREATE INDEX IF NOT EXISTS idx_csv_cache_log_fetched_at ON csv_cache_log(fetched_at DESC)`,
  ];
...
/** Returns the current cached CSV if successful, null otherwise. */
export async function getCachedCSV(): Promise<CachedCSV | null> {
  const db = await ensureSchema();
  const result = await db.execute({
    sql: `SELECT raw_text, bytes_size, fetched_at, source_url
          FROM csv_cache
          WHERE slot = 'current' AND success = 1
          LIMIT 1`,
    args: [],
  });
  if (result.rows.length === 0) return null;
  ...
  return {
    ...
    bytes_size: row.bytes_size as number,
    fetched_at: row.fetched_at as string,
    source_url: row.source_url as string,
  };
}

/** Returns the previous cached CSV — fallback when current is missing or failed. */
export async function getPreviousCachedCSV(): Promise<CachedCSV | null> {
  const db = await ensureSchema();
  const result = await db.execute({
    sql: `SELECT raw_text, bytes_size, fetched_at, source_url
          FROM csv_cache
          WHERE slot = 'previous' AND success = 1
          LIMIT 1`,
    args: [],
  });
  if (result.rows.length === 0) return null;
...
/**
 * Atomically rotate cache: previous ← current, current ← new data.
 * Batch for atomicity — prevents race if two cron instances overlap.
 */
export async function upsertCachedCSV(input: {
  raw_text: string;
  bytes_size: number;
  ...
  await db.batch(
    [
      // Step 1: copy current → previous (no-op if current row absent)
      {
      ...
      // Step 2: upsert new data into current
      {
        sql: `INSERT INTO csv_cache
                (slot, raw_text, bytes_size, fetched_at, fetch_duration_ms,
                 source_url, success, error_message, created_at, updated_at)
...
/** Append a lightweight audit row — never auto-deleted. */
export async function appendCacheLog(input: {
  ...
  const db = await ensureSchema();
  await db.execute({
    sql: `INSERT INTO csv_cache_log
            (bytes_size, fetch_duration_ms, success, error_message, source_url)
          VALUES (?, ?, ?, ?, ?)`,
    args: [
      input.bytes_size ?? null,
      input.fetch_duration_ms ?? null,
      input.success ? 1 : 0,
      input.error_message ?? null,
      input.source_url,
    ],
