...
function hasChanged(row: AosomProduct, snap: ProductSnapshot): boolean {
  ...
  // BUG-C-STEP3 (2026-05-07): "Low Stock Alert" string (not date-based), present on
  // only 204/10,731 products (1.9%). Not used business-side. Safe to exclude (same
  // rationale as BUG-C-STEP2 for estimatedArrival). Field still written on upsert
  // when a product changes for another reason.
