...
<style>
  ...
  .hv-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}
  ...
  .hv-card:hover .hv-ov,.hv-card:focus .hv-ov{opacity:1}
  .hv-t{color:#fff;font-weight:700;font-size:1.45rem;line-height:1.25}
  .hv-p{color:#D4A853;font-weight:700;font-size:1.4rem}
  @media(max-width:989px){.hv-grid{grid-template-columns:repeat(2,1fr)}}
  @media(max-width:600px){.hv-grid{grid-template-columns:1fr}.hv-ov{opacity:1}}
</style>
<script>
  (function(){
    var vids=document.querySelectorAll('.hv-vid');
    if(!vids.length) return;
    function activate(v){ var s=v.querySelector('source[data-src]'); if(s && !s.src){ s.src=s.getAttribute('data-src'); v.load(); } var pr=v.play(); if(pr&&pr.catch)pr.catch(function(){}); }
...
console.log("sections/home-video-showcase.liquid PUT 200");

// add to index.json after the carousels (after featured_collection2)
const idx = JSON.parse(await getAsset("templates/index.json", PREVIEW));
idx.sections.home_video = { type: "home-video-showcase", settings: {} };
if (!idx.order.includes("home_video")) {
  const pos = idx.order.indexOf("featured_collection2");
  if (pos >= 0) idx.order.splice(pos + 1, 0, "home_video");
  else idx.order.push("home_video");
}
await putAsset("templates/index.json", JSON.stringify(idx, null, 2), PREVIEW);
console.log("index.json PUT 200. order:", idx.order.join(", "));
