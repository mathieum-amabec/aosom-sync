// STEP 4-5: execute the pos-1 swaps from pos1-swap-plan-clean324.json.
// PUT each hero image to position 1, then verify via re-GET. Shopify 2 req/sec.
// Resumable (skip products already in the results checkpoint). Per-product errors are
// logged and skipped — never blocks the run. SAFETY: requires --apply to write.
//   node scripts/apply-pos1-swaps-v2.mjs --apply
...
let last = 0;
async function shop(method, path, body) {
  for (let a = 0; a < 6; a++) {
...
const plan = JSON.parse(readFileSync(new URL("../pos1-swap-plan-clean324.json", import.meta.url), "utf8"));
process.stderr.write(`${APPLY ? "*** APPLY" : "--- DRY-RUN"} — ${plan.length} swaps ***\n`);

const ckpt = new URL("../pos1-apply-v2.checkpoint.jsonl", import.meta.url);
const done = new Map();
if (existsSync(ckpt)) { for (const l of readFileSync(ckpt, "utf8").split(/\r?\n/)) { if (!l.trim()) continue; try { const o = JSON.parse(l); done.set(String(o.id), o); } catch {} } process.stderr.write(`resume: ${done.size} already processed\n`); }
  ...
  try {
    ...
    if (put.status !== 200) { rec.error = (await put.text()).slice(0, 200); process.stderr.write(`  [${i}/${plan.length}] ${p.handle} PUT ${put.status} FAIL\n`); }
      ...
      for (let a = 0; a < 4 && !verified; a++) {
        if (a > 0) await sleep(1000);
        const g = await shop("GET", `/products/${p.id}/images.json`);
        if (g.status !== 200) continue;
        const imgs = ((await g.json()).images || []).sort((x, y) => x.position - y.position);
        const pos1 = imgs.find((im) => im.position === 1);
        now = pos1 ? pos1.id : null;
        verified = pos1 && String(pos1.id) === String(p.hero_image_id);
      }
    ...
    rec.put_status = "ERROR"; rec.error = String(e.message).slice(0, 200);
    process.stderr.write(`  [${i}/${plan.length}] ${p.handle} ERROR ${e.message}\n`);
  }
  done.set(String(p.id), rec);
  appendFileSync(ckpt, JSON.stringify(rec) + "\n");
}

// report (only when all processed)
if (done.size >= plan.length) {
  ...
  const swappedTwice = ids.filter((v, idx) => ids.indexOf(v) !== idx);
  const report = {
    generated_from: "pos1-swap-plan-clean324.json",
    total: plan.length,
    put_200: ok.length,
    verified: verified.length,
    failed: failed.length,
    swapped_twice: swappedTwice.length,
    failures: failed.map((r) => ({ id: r.id, handle: r.handle, status: r.put_status, error: r.error })),
  };
  writeFileSync(new URL("../pos1-swap-report-v2.json", import.meta.url), JSON.stringify(report, null, 2));
  process.stderr.write(`\n=== REPORT === PUT200 ${ok.length}/${plan.length} | verified ${verified.length} | failed ${failed.length} | swapped_twice ${swappedTwice.length}\n`);
  console.log(JSON.stringify(report, null, 2));
} else {
  process.stderr.write(`\nINCOMPLETE: ${done.size}/${plan.length} — rerun to resume\n`);
}
