// Chantier 3 — hero button visibility fix, PREVIEW theme only (160213696617).
// Buttons were disappearing into the hero photo. Fix: solid navy primary + gold border,
...
const HERO = `{%- assign loc = request.locale.iso_code | downcase -%}<div class="lc-hero" style="position:relative;background:url('{{ 'lc-hero.jpg' | asset_url }}') center/cover no-repeat;min-height:480px;display:flex;align-items:flex-start">
  <div class="lc-hero-ov" style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.6) 0%,rgba(0,0,0,.2) 45%,transparent 70%),linear-gradient(to right,rgba(0,0,0,.55) 0%,rgba(0,0,0,.22) 60%,transparent 100%),linear-gradient(to top,rgba(0,0,0,.45) 0%,rgba(0,0,0,.12) 22%,transparent 42%)"></div>
  <div class="page-width lc-hero-in" style="position:relative;color:#fff;max-width:660px;font-family:'DM Sans',sans-serif">
    ...
    <div class="lc-hero-cta">
      ...
      <a href="/collections/rabais" class="lc-btn lc-btn--ghost" data-umami-event="Hero CTA deals">{% if loc == 'en' %}See the deals{% else %}Voir les rabais{% endif %}</a>
    </div>
  </div>
</div>
<style>
  ...
  .lc-hero h1{font-size:clamp(2.4rem,5vw,4.4rem);line-height:1.08;margin:0 0 1rem;font-weight:700;text-shadow:0 2px 12px rgba(0,0,0,.5)}
  .lc-hero p{font-size:1.7rem;margin:0 0 2rem;max-width:34ch;text-shadow:0 1px 8px rgba(0,0,0,.55)}
  .lc-hero-cta{display:flex;gap:14px;flex-wrap:wrap}
  .lc-btn{display:inline-block;padding:14px 28px;border-radius:8px;font-weight:700;text-decoration:none;font-size:1.5rem;font-family:'DM Sans',sans-serif;transition:transform .15s ease,background .15s ease,color .15s ease;box-shadow:0 4px 16px rgba(0,0,0,.28)}
  .lc-btn--navy{background:#1B2A4A;color:#fff;border:2px solid #C17F3E}
  .lc-btn--navy:hover{background:#16223c;transform:translateY(-2px)}
  .lc-btn--ghost{background:rgba(255,255,255,.85);color:#1B2A4A;border:2px solid #C17F3E}
  .lc-btn--ghost:hover{background:#fff;transform:translateY(-2px)}
  @media(max-width:749px){.lc-hero-in{text-align:center;margin:0 auto;padding:16% 0 40px}.lc-hero h1{font-size:clamp(1.9rem,6vw,2.6rem)}.lc-hero p{margin-left:auto;margin-right:auto}.lc-hero-cta{justify-content:center}}
</style>`;
...
console.log(`✔ hero buttons fix — PUT templates/index.json → HTTP ${status}`);
