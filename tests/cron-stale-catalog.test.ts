...
describe("GET /api/cron/stale-catalog", () => {
  beforeEach(() => {
    runMock.mockReset().mockResolvedValue({ stale: 106, drafted: 103, skipped: 1, excluded: 0, failed: 2, deferred: 0 });
  ...
  it("records 'stale=N drafted=X skipped=Y excluded=W failed=Z deferred=D' and returns the result", async () => {
    const res = await GET(auth());
    expect(res.status).toBe(200);
    expect(recMock).toHaveBeenCalledWith("stale-catalog", "success", "stale=106 drafted=103 skipped=1 excluded=0 failed=2 deferred=0");
    expect(await res.json()).toEqual({ success: true, stale: 106, drafted: 103, skipped: 1, excluded: 0, failed: 2, deferred: 0 });
  });
