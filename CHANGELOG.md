...
- **Catalog-freshness zero-out** (`runCatalogFreshnessZero`, `removed-catalog.ts`) — any
  ...
  process. Non-fatal, independent of today's diff.
...
### Dry-run (production, 2026-09-20, threshold=21 days)

- **3,218 products** would be zeroed on first run: 3,168 never imported + 50 already
  imported (persistent stragglers the same-day reconcile's rename-suspect guard leaves
  live — this rule is a slightly earlier catch of what the existing 30-day stale-catalog
  net would eventually draft anyway).
- Age breakdown of the affected set: 1,519 rows >90 days stale, 1,016 rows 60-90 days,
  683 rows 21-60 days.
- Applied to production only after operator sign-off on these numbers.
